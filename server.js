...
if (vagueRecords.length > 0) {
  ...
  // === SOLO SI HAY DATOS, SE DIBUJA LA TABLA ===
  ...
  vagueRecords.forEach((row, i) => {
    ...
    let x = positions[0];
    columns.forEach((cw) => {
      strokeRect(doc, x, y, cw, rowHeight);
      x += cw;
    });
...
if (tellingRecords.length > 0) {
  ...
  tellingMatrix.forEach((row, i) => {
    ...
    let tx2 = tPos[0];
    Object.values(tCols).forEach((cw) => {
      strokeRect(doc, tx2, ty, cw, tRowH);
      tx2 += cw;
    });
  ...
  const totalWidth = Object.values(tCols).slice(0, 5).reduce((a, b) => a + b, 0);

  fillRect(doc, tPos[0], ty, totalWidth + tCols.valor, tRowH, "#e6e6e6");
  strokeRect(doc, tPos[0], ty, totalWidth, tRowH);
  strokeRect(doc, tPos[5], ty, tCols.valor, tRowH);
